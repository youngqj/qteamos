...
public class DynamicClassLoaderFactory {
     ...
     */
    public DynamicClassLoader getClassLoader(String pluginId) {
    ...
    public void releaseClassLoader(String pluginId) {
        ...
        if (classLoader != null && !classLoader.isClosed()) {
            try {
                classLoader.close();
                log.info("关闭插件类加载器: pluginId={}", pluginId);
            } catch (IOException e) {
                log.error("关闭插件类加载器时出错: pluginId={}, error={}", pluginId, e.getMessage(), e);
            }
        }
    }
        ...
        for (Map.Entry<String, DynamicClassLoader> entry : classLoaderCache.entrySet()) {
            ...
            if (classLoader != null && !classLoader.isClosed()) {
                try {
                    ...
                    log.error("关闭插件类加载器时出错: pluginId={}, error={}", 
    ...
    public boolean hasClassLoader(String pluginId) {
        return classLoaderCache.containsKey(pluginId);
    }
